...
--Запрос 3: Выбрать id регионов и максимальную дату основания санаториев, сгруппированных по региону для регионов, 
--максимальную дата основания санаториев в которых > 2002.
select reg_id, max(fyear)
from sanat 
group by reg_id
having max(fyear) > 2002;


--Задание 3

CREATE VIEW sanatnames AS
    SELECT name
    FROM sanat;
--drop view sanatnames;
select * from sanatnames;
...
drop procedure dropviews;

create or replace procedure dropviews(a inOUT INT)
as $$
begin
    a := 5;
